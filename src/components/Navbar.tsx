...
export default function Navbar() {
  ...
  return (
    <>
      <nav
        className={`fixed top-6 left-1/2 -translate-x-1/2 z-50 flex items-center justify-between px-6 py-3 w-[calc(100%-48px)] max-w-5xl rounded-full transition-[background-color,border-color,box-shadow,backdrop-filter] duration-300 navbar-fade opacity-0 ${scrolled
          ...
          <div className="relative w-full max-w-sm bg-[#131316] border border-white/10 rounded-3xl p-8 shadow-2xl z-10 animate-scale-up text-left overflow-hidden">
            ...
            <form
              onSubmit={async (e) => {
                ...
                link.href = "/Echo_v0.1.zip";
                link.setAttribute("download", "Echo_v0.1.zip");
